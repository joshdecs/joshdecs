# 👋 Hi, I'm Josh!

I'm a **Software Engineering student at Polytechnique Montréal**, passionate about **AI development, software design**, and creating **practical digital solutions** that make everyday life easier.

I love building small, meaningful projects — tools that solve real problems I encounter in my studies, my daily life, and my creative pursuits. Whether it’s managing my music library, automating repetitive tasks, or simplifying access to university resources, I’m always looking for ways to combine **creativity** and **engineering**.

---

### 🧠 About Me
- 🎓 **B.Eng. in Software Engineering**, Polytechnique Montréal  
- 💡 Passionate about **AI, software development**, and **problem-solving**  
- 🎵 Interested in **building tech tools** that connect software and creativity (especially in music)  
- 🧩 Developing **university productivity tools** to improve student workflows  
- 🌍 Open to **internships** and **collaborative projects** in **software engineering or AI-driven development**

---

### 🛠️ Tech Stack
- **Languages:** Python, JavaScript, HTML, CSS  
- **Tools:** Git, VS Code, Linux, Docker (learning)  
- **Areas of Interest:** Machine Learning, Backend Development, and Creative Coding

---

### 🚀 What I’m Working On
- 🎧 Tools to automate and organize music downloads and management  
- 🏫 Apps that make university life easier (e.g., room availability, class utilities)  
- 🤖 Learning how to integrate **AI models** into practical applications  
- 🧠 Continuously improving my understanding of **software architecture** and **clean development practices**

---

### 📫 Get in Touch
- 💼 [LinkedIn]([https://www.linkedin.com/](https://www.linkedin.com/in/joshua-dechesne-984872306/))   
- 📧 [Email me](mailto:joshua.dechesne@gmail.com)

---


